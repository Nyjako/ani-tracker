...
pub struct MAL {
    pub client: OauthClient<Authenticated>
}

impl MAL {
    pub async fn new() -> MAL {
        if let Some(auth) = MAL::try_conf_login().await {
            return MAL {
                client: auth,
            };
        }
        ...
        let code = match MAL::catch_callback().await {
            Ok(code) => code,
            Err(err) => {
        ...
        MAL {
            client: authenticated_oauth_client,
        }
    }
